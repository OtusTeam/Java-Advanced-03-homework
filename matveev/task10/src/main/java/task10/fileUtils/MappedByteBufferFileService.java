...
public class MappedByteBufferFileService implements FileService{

    public Cache cache;

    public MappedByteBufferFileService(Cache cache) {
        this.cache = cache;
    }
    ...
    public void put(String fileName) {
        try {
            Buffer buffer = fileReader.readMappedByteBufferFromFile(fileName);
            cache.put(fileName, buffer);
            System.out.println("Added " + fileName + " to cache via MappedByteBuffer with content: " + buffer);
        } catch (Exception e) {
